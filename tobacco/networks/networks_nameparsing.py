...
def parse_name(name_raw: str) -> HumanName:
    ...
    >>> n.last, n.first, n.middle, n.position
    ('Teague', 'C', 'E', 'JR')
    ...
    >>> n.last, n.first, n.middle, n.position
    ('Henson', 'A', '', 'AMERICAN OUTSIDE COUNSEL) (HANDWRITTEN NOTES CHADBOURNE, PARKE, WHITESIDE & WOLFF')
    ...
    if name.suffix:
        extracted_positions = [name.suffix] + extracted_positions
    #print("ex", extracted_positions)
    name.position = " ".join(extracted_positions).upper()
...
class Person:
    ...
    def __init__(self, last, first, middle='', position='', aliases=[], count=0):
        self.last = last
        self.first = first
        self.middle = middle
    ...
    def copy(self):
        return Person(last=self.last, first=self.first, middle=self.middle, position=self.position,
                      aliases=copy.deepcopy(self.aliases), count=self.count)

    def __hash__(self):
        return hash(f'{self.last} {self.first} {self.middle} {self.position}')
...
#                    print('single first name', person2)
    ...
    def merge_two_persons(self, person1, person2):
        ...
        for attr in ['first', 'middle', 'position']:
